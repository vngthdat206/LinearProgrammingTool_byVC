...
if __package__ in (None, ""):
    ...
    from utils import fr, sense_to_standard
else:
    from .models import ProblemData, Snapshot, PivotStep, SolveTrace, SolveReport
    from .utils import fr, sense_to_standard
...
class SimplexEngine:
    ...
    def _transform_variables(self) -> None:
        ...
        if self.problem.objective_sense == "max":
            ...
            std_obj = raw_obj[:]
            self.standardization_lines.append("Bài toán giữ nguyên dạng min để giải bằng đơn hình.")
        ...
        for idx, sign in enumerate(self.problem.var_signs):
            name = f"x{idx+1}"
            if sign in (">=0", ">0"):
                # FIX: Giữ nguyên tên x{idx+1} thay vì đổi sang y{j+1}
                j = len(self.std_names)
                self.std_names.append(name)
                self.variable_mapping.append([(j, Fraction(1))])
                if sign == ">0":
                    self.strict_notes.append(
                        f"Biến {name} được chọn > 0; app chuẩn hóa về biến không âm {name} ≥ 0 để giải bằng đơn hình."
                    )
                self.standardization_lines.append(f"  {name} = {name}, với {name} ≥ 0")
            elif sign in ("<=0", "<0"):
                j = len(self.std_names)
                y_name = f"y{idx+1}"
                self.std_names.append(y_name)
                self.variable_mapping.append([(j, Fraction(-1))])
                self.standardization_lines.append(f"  {name} = -{y_name}  ⇒ {y_name} ≥ 0")
                self.strict_notes.append(
                    f"Biến {name} có điều kiện {sign}; app đặt {y_name} = -{name} để đưa về biến không âm."
                )
            elif sign == "tự do":
                j1 = len(self.std_names)
                y1_name = f"y{idx+1}+"
                self.std_names.append(y1_name)
                j2 = len(self.std_names)
                y2_name = f"y{idx+1}-"
                self.std_names.append(y2_name)
                self.variable_mapping.append([(j1, Fraction(1)), (j2, Fraction(-1))])
                self.standardization_lines.append(
                    f"  {name} = {y1_name} - {y2_name}, với {y1_name} ≥ 0, {y2_name} ≥ 0"
                )
                self.strict_notes.append(
                    f"Biến {name} là biến tự do nên app đặt {name} = {y1_name} - {y2_name} để giải bằng đơn hình."
                )
            else:
        ...
        self.standardization_lines.append("Biến sau khi chuẩn hóa:")
        for orig_idx, mapping in enumerate(self.variable_mapping):
            lhs = f"x{orig_idx+1}"
            if len(mapping) == 1:
                k, coef = mapping[0]
                if coef == 1:
                    self.standardization_lines.append(f"  {lhs} = {self.std_names[k]}")
                else:
                    self.standardization_lines.append(f"  {lhs} = -{self.std_names[k]}")
            else:
                k1, c1 = mapping[0]
                ...
                sign = "+" if c2 == 1 else "-"
                self.standardization_lines.append(
                    f"  {lhs} = {self.std_names[k1]} {sign} {self.std_names[k2]}"
                )
    ...
    def _transform_constraints(self) -> None:
        for i, cons in enumerate(self.problem.constraints):
            ...
            if sense in ("<", ">"):
                self.strict_notes.append(
                    f"Ràng buộc {i+1} dùng dấu chặt '{sense}' nên app quy đổi về dạng không chặt tương ứng trước khi giải."
                )
                sense = "<=" if sense == "<" else ">="

            # FIX: Chuyển đổi >= về <= bằng cách nhân -1 với tất cả hệ số
            if sense == ">=":
                row = [-a for a in row]
                rhs = -rhs
                sense = "<="
                self.standardization_lines.append(
                    f"  Ràng buộc {i+1} là dạng >=, nhân với -1 để đưa về dạng <= chuẩn."
                )

            # Giữ nguyên rhs âm nếu ràng buộc vẫn đang ở dạng <=.
            # Trường hợp này sẽ được xử lý bằng pha 1 với biến phụ x0.
            if sense == "<=" and rhs < 0:
                self.standardization_lines.append(
                    f"  Ràng buộc {i+1} có b < 0 nhưng vẫn là dạng <= chuẩn, giữ nguyên để xử lý bằng pha 1 nếu cần."
                )

            self.std_constraints.append(row)
            self.std_senses.append(sense_to_standard(sense))
    ...
    def _build_dictionary(self) -> None:
        ...
        for coeffs, sense, b in zip(self.std_constraints, self.std_senses, self.std_rhs):
            ...
            if sense == "<=":
                ...
                aidx = len(self.all_names)
                self.all_names.append(f"a{next_art}")
                next_art += 1
                basis.append(aidx)
                rows.append(row)
                rhs.append(b)
                art_vars.append(aidx)
            else:
        ...
        self.artificial_vars = art_vars
        self.need_aux_phase1 = (not self.artificial_vars) and any(b < 0 for b in self.initial_rhs)
    ...
    # ---------- solving ----------
    def _choose_entering(self, obj: Dict[int, Fraction], method: str) -> Optional[int]:
        neg_vars = [j for j, c in obj.items() if c < 0]
        if not neg_vars:
            return None
        if method == "dantzig":
    ...
    def _solve_once(self, method: str, phase: int, basis: List[int], rows: List[Dict[int, Fraction]], rhs: List[Fraction], obj_const: Fraction, obj: Dict[int, Fraction], objective_label: str, art_vars: List[int]) -> SolveTrace:
        ...
        for iteration in range(1, max_iter + 1):
            ...
            entering = self._choose_entering(obj, method)
            if entering is None:
                # optimal
    ...
    def solve_full(self) -> SolveReport:
        ...
        if self.need_aux_phase1:
            ...
            if phase1.status == "cycle":
                bland1 = self._solve_phase1_aux_once("bland", basis, rows, rhs, obj_const, obj, "δ", aux_idx)
                if bland1.status == "cycle":
                    return self._assemble_report("bland", phase1, bland1, notes, phase1_infeasible=False)
                phase1 = bland1
                used = "bland"
            else:
                used = "dantzig"
            ...
            if phase1.final_snapshot is None:
                return self._assemble_report(used, phase1, None, notes, phase1_infeasible=True)
            ...
            return self._assemble_report(used, phase1, phase2, notes, phase1_infeasible=False)
        ...
        if self.artificial_vars:
            ...
            if phase1.status == "cycle":
                bland1 = self._solve_once("bland", 1, basis, rows, rhs, obj_const, obj, "w", self.artificial_vars)
                if bland1.status == "cycle":
                    return self._assemble_report("bland", phase1, bland1, notes, phase1_infeasible=False)
                phase1 = bland1
                used = "bland"
        ...
        if dantzig.status == "cycle":
            ...
            return self._assemble_report("bland", dantzig, bland, notes, phase1_infeasible=False)
        return self._assemble_report("dantzig", dantzig, None, notes, phase1_infeasible=False)

    def _assemble_report(self, used_method: str, dantzig: SolveTrace, bland: Optional[SolveTrace], notes: List[str], phase1_infeasible: bool) -> SolveReport:
        trace = bland if (bland is not None and bland.status != "cycle") else dantzig
        final_snapshot = trace.final_snapshot
        solution_std, solution_orig, objective_std, objective_orig, multiple = self.extract_solution(final_snapshot) if final_snapshot else ({}, {}, None, None, False)
        status = trace.status
    ...
    def extract_solution(self, snapshot: Snapshot) -> Tuple[Dict[int, Fraction], Dict[int, Fraction], Optional[Fraction], Optional[Fraction], bool]:
        ...
        multiple = any(j not in snapshot.basis and snapshot.obj.get(j, Fraction(0)) == 0 for j in snapshot.obj.keys())
        return std_values, orig_values, obj_std, obj_orig, multiple
